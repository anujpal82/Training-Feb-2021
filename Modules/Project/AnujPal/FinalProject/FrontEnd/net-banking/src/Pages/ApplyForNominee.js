...
export const ApplyForNominee = (props) => {
  ...
  const [formError, setFormError] = useState({ accountNoError: "", nameError: "", DOBError: "", relationError: "", equityError: "" })
  const [formFlag, setFormFlag] = useState({ accountNoFlag: "", nameFlag: "", DOBFlag: "", relationFlag: "", equityFlag: "" })
  ...
  const formValidation = (e) => {
    ...
    switch (name) {
      case 'accountNo':
        if (value === "default") {
          setFormError({ ...formError, accountNoError: "please select from the above dropdown" })
      ...
      case 'name':
        if (value.length < 3) {
          setFormError({ ...formError, nameError: "name should have atleast 3 characters" })
      ...
      case 'DOB':
        if (value === "") {
          setFormError({ ...formError, DOBError: "please select date of birth" })
      ...
      case 'relation':
        if (value === "default") {
          setFormError({ ...formError, relationError: "please select relation from above dropdown" })
    ...
    <>
      ...
      <div className="col-md-6 mx-auto mt-5">
        <form className="bg-light p-4 m-4 mt-5" id="signUpForm">
          ...
          >
            <option value="default">Select Account Number</option>
            {CRN.map((item) => {
              return <option>{item.accountNo}</option>;
            })}
          </select>
          <span className="text-danger"><small>{formError.accountNoError}</small></span>
          ...
          <input
            ...
            className="form-control"
          />
          <span className="text-danger"><small>{formError.nameError}</small></span>
          ...
          <input
            ...
            className="form-control"
          />
          <span className="text-danger"><small>{formError.DOBError}</small></span>
          ...
          <span className="text-danger"><small>{formError.relationError}</small></span>
          ...
          <select
            ...
            name="equity"
          >
            <option value="default">% of Equity Want to Give</option>
            <option>25%</option>
            <option>50%</option>
            <option>75%</option>
            <option>100%</option>
          </select>
          <span className="text-danger"><small>{formError.equityError}</small></span>

          <div className="mt-4" id="recaptcha-container"></div>
          <div className="mt-3">
            <button className="btn btn-secondary w-100 " disabled={formFlag.accountNoFlag === true && formFlag.nameFlag === true && formFlag.DOBFlag === true && formFlag.relationFlag === true && formFlag.equityFlag === true ? false : true} onClick={Apply}>
              Apply
            </button>
          </div>
        </form>
